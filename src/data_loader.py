...
class MSADataset(Dataset):
    def __init__(self, config, args):
        ...
        if args.dataset=='mosi':
            ...
            print("Dataset not defined correctly. Please choose from 'mosi' and 'mosei'")
            exit()
...
def get_loader(hp, config, shuffle=True): 
    ...
    def collate_fn(batch):
        ...
        for sample in batch:
            ...
            # print(sample[1])
            labels.append(torch.from_numpy(sample[1]))  # MOSI, MOSEI
            # labels.append(sample[1])  # SIMS
            ids.append(sample[2])
        vlens = torch.cat(v_lens)
        alens = torch.cat(a_lens)
        labels = torch.cat(labels, dim=0) # MOSI, MOSEI
        ...
        for sample in batch:
            ...
            encoded_bert_sent = bert_tokenizer.encode_plus(
                text, max_length=SENT_LEN, add_special_tokens=True, truncation=True, padding='max_length', pad_to_max_length=True)
